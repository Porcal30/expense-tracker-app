  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_isEditMode) {
        ...
        saveFuture = provider.updateExpense(updatedExpense);
      } else {
        final expense = Expense(
          id: const Uuid().v4(),
        ...
        saveFuture = provider.addExpense(expense);
      }

      // Fire-and-forget so offline local writes do not trap the user on this page.
      unawaited(
        saveFuture.catchError((error) {
      ...
      messenger.showSnackBar(
        SnackBar(
          content: Text(
            _isEditMode
                ? 'Expense updated. It will sync when online if needed.'
                : 'Expense added. It will sync when online if needed.',
          ),
            ...
            child: Column(
              ...
              children: [
                AppTextField(
                  ...
                  enabled: !_isSaving,
                  validator: (value) =>
                      Validators.requiredText(value, field: 'Title'),
                ),
